...
let inputElem = document.getElementById('amount')
let boxText = document.querySelector('#result')


btn.addEventListener('click' , function (){
  let lenghtInput = inputElem.value
  if (lenghtInput > text.length){
    

  }
  if( lenghtInput === '' || isNaN(lenghtInput)){
    boxText.innerHTML = text[1]
  }else {
    let sliceText = text.slice( 0 , lenghtInput)
    let mapText = sliceText.map(function (texts){
      return '<p>'+ texts +'</p>'
    })
    boxText.innerHTML = mapText
}
inputElem.value = ''
